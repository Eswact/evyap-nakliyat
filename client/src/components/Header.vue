...
<template>
    <div class="h-6 md:hidden w-full bg-main/20 dark:bg-bg/20 flex justify-center items-center">
        <div class="w-full max-w-[1600px] px-[20px] flex justify-between items-center">
            <span></span>
            <a class="text-sm font-bold text-text dark:text-white" href="tel:0216 573 71 34">Tel : 0532 472 05 90</a>
        </div>
    </div>
    <div class="w-full h-[calc(100%-1.5rem)] md:h-full flex justify-between items-center max-w-[1600px] py-[12px] px-[20px] lg:px-[10px]">
        <nav class="flex items-end gap-[20px] xl:gap-[10px] text-text dark:text-white md:hidden">
            ...
            <RouterLink :class="{ 'text-main': isActive('/hizmetler') }" class="py-[6px] px-[12px] xl:px-[6px] flex items-center gap-[8px] relative group font-ptSans font-semibold" to="/hizmetler"> 
                <span class="text-[1.2rem] font-semibold">Hizmetlerimiz</span>
                <span :class="{ 'w-full': isActive('/hizmetler'), 'w-0': !isActive('/hizmetler') }" class="absolute left-0 bottom-1 h-[2px] bg-main transition-all duration-200 group-hover:w-full"></span>
            </RouterLink> 
            <RouterLink :class="{ 'text-main': isActive('/hakkinda') }" class="py-[6px] px-[12px] xl:px-[6px] flex items-center gap-[8px] relative group font-ptSans font-semibold" to="/hakkinda"> 
                <span class="text-[1.2rem] font-semibold">Hakkımızda</span>
                <span :class="{ 'w-full': isActive('/hakkinda'), 'w-0': !isActive('/hakkinda') }" class="absolute left-0 bottom-1 h-[2px] bg-main transition-all duration-200 group-hover:w-full"></span>
            </RouterLink>   
            <RouterLink :class="{ 'text-main': isActive('/iletisim') }" class="py-[6px] px-[12px] xl:px-[6px] flex items-center gap-[8px] relative group font-ptSans font-semibold" to="/iletisim"> 
                <span class="text-[1.2rem] font-semibold">İletişim</span>
                <span :class="{ 'w-full': isActive('/iletisim'), 'w-0': !isActive('/iletisim') }" class="absolute left-0 bottom-1 h-[2px] bg-main transition-all duration-200 group-hover:w-full"></span>
            </RouterLink>   
        </nav>
